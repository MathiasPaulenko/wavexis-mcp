...
| `wavexis_extract` | `url, json_schema, selector?, session_id?, wait_timeout?, headless?, backend?` | Extract structured data from a page using a JSON schema and optional CSS selector. |
| `wavexis_lighthouse` | `url, categories?, session_id?, wait_timeout?, headless?, backend?` | Run a Lighthouse-style audit on a URL. |
| `wavexis_record` | `session_id, url, duration?, headless?, backend?` | Record browser interactions and generate a YAML workflow. |
