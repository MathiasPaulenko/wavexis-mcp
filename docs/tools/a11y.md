...
| `wavexis_axe_audit` | `session_id`, `selector` | Run an axe-core accessibility audit. Returns violations categorized by impact (critical, serious, moderate, minor) with element selectors and remediation suggestions. Optional `selector` to scope the audit. |
